...
@dataclass(frozen=True, kw_only=True)
class SPARKINFERMHCScratchCaps:
...
def _use_mhc_prefill_bf16_project(
    *,
    ...
    return (
        ...
        and os.environ.get("SPARKINFER_MHC_PREFILL_BF16_MMA", "1") != "0"
    )
...
def _layout_mhc_scratch(caps: SPARKINFERMHCScratchCaps) -> _MHCScratchLayout:
    cursor = 0
...
def plan_mhc_scratch(caps: SPARKINFERMHCScratchCaps) -> SPARKINFERMHCScratchPlan:
    layout = _layout_mhc_scratch(caps)
    return SPARKINFERMHCScratchPlan(
        caps=caps,
        layout=layout,
        _scratch_specs=(
            scratch_buffer_spec(
...
def _sparkinfer_mhc_pre_impl(
    ...
    *,
    ...
    # See sparkinfer_mhc_post_pre: caller-owned buffers -> mutating ops; no buffers
    ...
    raise ValueError(
        "sparkinfer_mhc_pre is served only by the fused Gram kernel, which "
...
def _sparkinfer_mhc_post_pre_impl(
    x: torch.Tensor,
    residual: torch.Tensor,
    prev_post: torch.Tensor,
    prev_comb: torch.Tensor,
    ...
    *,
    ...
    if (
        ...
        use_prefill_compact = (
            ...
            and norm_weight is not None
            and policy_m >= prefill_min_tokens
            and os.environ.get("SPARKINFER_MHC_PREFILL_COMPACT", "1") != "0"
        )
    ...
    raise ValueError(
        "sparkinfer_mhc_post_pre is served only by the fused Gram kernel, which "
        "supports the decode config "
        f"(hidden_size divisible by {MHC_GRAM_BLOCK_H}, "
        f"split_k=hc_mult*hidden_size/{MHC_DEFAULT_BLOCK_K}, "
        f"block_k={MHC_DEFAULT_BLOCK_K}, block_h={MHC_DEFAULT_BLOCK_H}, "
...
@torch.library.custom_op(
    "sparkinfer::mhc_pre_planned_functional",
    mutates_args=(),
)
def _mhc_pre_planned_functional_op(
    ...
    return _sparkinfer_mhc_pre_impl(
        residual,
        fn,
        hc_scale,
        hc_base,
...
@torch.library.custom_op(
    "sparkinfer::mhc_post_pre_planned_functional",
    mutates_args=(),
)
def _mhc_post_pre_planned_functional_op(
    ...
    return _sparkinfer_mhc_post_pre_impl(
        x,
        residual,
        prev_post,
        prev_comb,
...
@torch.library.custom_op(
    "sparkinfer::mhc_post_planned_functional",
    mutates_args=(),
)
def _mhc_post_planned_functional_op(
    ...
    return _sparkinfer_mhc_post_impl(
        x,
        residual,
        prev_post,
        prev_comb,
...
def sparkinfer_mhc_pre(
    ...
    *,
    ...
    if compiling and no_caller_owned_buffers:
        ...
        return torch.ops.sparkinfer.mhc_pre_planned_functional(
            residual,
            fn,
            hc_scale,
            hc_base,
    ...
    if compiling:
        raise RuntimeError(
            "sparkinfer_mhc_pre must be opaque to torch.compile; caller-owned mHC "
...
def sparkinfer_mhc_post_pre(
    ...
    *,
    ...
    if compiling and no_caller_owned_buffers:
        ...
        norm_weight_for_kernel = norm_weight if norm_weight is not None else residual
        return torch.ops.sparkinfer.mhc_post_pre_planned_functional(
            x,
            residual,
            prev_post,
            prev_comb,
    ...
    if compiling:
        raise RuntimeError(
            "sparkinfer_mhc_post_pre must be opaque to torch.compile; caller-owned "
...
def sparkinfer_mhc_post(
    ...
    *,
    ...
    if compiling and out is None:
        return torch.ops.sparkinfer.mhc_post_planned_functional(
            x,
            residual,
            prev_post,
            prev_comb,
    ...
    if compiling:
        raise RuntimeError(
            "sparkinfer_mhc_post must be opaque to torch.compile; caller-owned mHC "
