...
_WO_QUANT_CHUNKS_PER_PROGRAM = int(
    os.environ.get("SPARKINFER_WO_QUANT_CHUNKS_PER_PROGRAM", "16")
)
if _WO_QUANT_CHUNKS_PER_PROGRAM not in (1, 2, 4, 8, 16, 32):
    raise ValueError(
        "SPARKINFER_WO_QUANT_CHUNKS_PER_PROGRAM must be one of 1, 2, 4, 8, 16, or 32, got "
...
def pack_mxfp8_scales_for_dense_gemm(
    ...
    *,
    ...
    """Pack compact MXFP8 row/chunk scales into sparkinfer dense-GEMM MMA layout.
...
def _requantize_block_fp8_to_ue8m0(
    ...
    *,
    ...
    block_amax / 448` is an *arbitrary* fp32 128x128 block scale. The sparkinfer MMA
...
def pack_wo_projection_fp8_block_scaled_weights_mxfp8(
    ...
    *,
    ...
    """Pack local DSV4 WO-A/WO-B checkpoint FP8 weights for the sparkinfer WO path."""
...
@torch.library.custom_op(
    "sparkinfer::quantize_wo_a_input_inv_rope_mxfp8_alloc",
    mutates_args=(),
)
...
@torch.library.custom_op(
    "sparkinfer::quantize_wo_a_input_inv_rope_mxfp8_launch",
    ...
    *,
    ...
    if out is None:
        values_base, scale_rows_base, scale_physical_base = (
            torch.ops.sparkinfer.quantize_wo_a_input_inv_rope_mxfp8_alloc(
                o,
                positions,
                cos_sin_cache,
                tokens,
    ...
    _check_mxfp8_rows_storage(out, m=tokens, k=group_width, num_groups=groups)
    torch.ops.sparkinfer.quantize_wo_a_input_inv_rope_mxfp8_launch(
        o,
        positions,
        cos_sin_cache,
        out.values,
...
@torch.library.custom_op(
    "sparkinfer::quantize_wo_b_input_mxfp8_alloc",
    mutates_args=(),
)
...
@torch.library.custom_op(
    "sparkinfer::quantize_wo_b_input_mxfp8_launch",
    mutates_args=("out_values", "out_scale_rows", "out_scale_mma"),
)
...
def quantize_wo_b_input_mxfp8(
    ...
    *,
    ...
    if out is None:
        values_base, scale_rows_base, scale_physical_base = (
            torch.ops.sparkinfer.quantize_wo_b_input_mxfp8_alloc(
                source_trg,
                tokens,
                rank,
                groups,
    ...
    torch.ops.sparkinfer.quantize_wo_b_input_mxfp8_launch(
        source_trg,
        out.values,
        out.scale_rows,
        out.scale_mma,
...
@torch.library.custom_op(
    "sparkinfer::wo_projection_inv_rope_mxfp8_fused",
    mutates_args=(),
)
