...
def _check_block_size(block_size: Sequence[int]) -> tuple[int, int]:
    ...
    if (block_n, block_k) != (128, 128):
        raise ValueError(
            f"sparkinfer block FP8 linear currently supports 128x128 weight blocks, got {block_size}"
        )
...
def pack_block_fp8_linear_weight_mxfp8(
    ...
    *,
    ...
    """Pack serialized block-FP8 linear weights for the native sparkinfer MXFP8 GEMM.
...
@torch.library.custom_op(
    "sparkinfer::quantize_block_fp8_linear_input_mxfp8_alloc",
    mutates_args=(),
)
    ...
    *,
    ...
    if out is None:
        ...
        values_base, scale_rows_base, scale_physical_base = (
            torch.ops.sparkinfer.quantize_block_fp8_linear_input_mxfp8_alloc(
...
@torch.library.custom_op(
    "sparkinfer::block_fp8_linear_mxfp8_fused",
    mutates_args=(),
)
...
def block_fp8_linear_mxfp8(
    ...
    *,
    ...
    """Run a serialized block-FP8 linear through the native sparkinfer MXFP8 GEMM.
    ...
    if binding is None:
        ...
        output = torch.ops.sparkinfer.block_fp8_linear_mxfp8_fused(
            source_2d,
    ...
    t_quant = time.perf_counter() if _SPARKINFER_TIMING else 0.0
    output = dense_gemm(
    ...
    t_gemm = time.perf_counter() if _SPARKINFER_TIMING else 0.0
    if bias is not None:
        output += bias
    if _SPARKINFER_TIMING:
        ...
        if total_ms >= _SPARKINFER_TIMING_THRESHOLD_MS:
            logger.warning(
                "sparkinfer_block_fp8_linear timing tokens=%d in=%d out=%d "
