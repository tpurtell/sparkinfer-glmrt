...
## Fork-native cache producers

`attention.dsv4_compressor` and `attention.dsv4_producer` retain their existing
local capacity plans and caller-owned arenas. They do not use a preparation
selection cache, and their local defaults are not measured upstream winners.
Their native export and graph-replay contracts require separate qualification.
