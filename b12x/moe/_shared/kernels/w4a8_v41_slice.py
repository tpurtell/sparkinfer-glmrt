...
This low-level kernel is not a serving binding or a replacement planner policy.
"""
...
class V41FusedSliceKernel:
    ...
    def __call__(
        ...
        self.kernel(x, xs, w13, s13, w2, s2, routing, out, rows).launch(
            grid=(self.slices, 1, 1), block=(128, 1, 1), stream=stream
        )
    ...
    def kernel(
        ...
        for kt in range(40):
            stage_repacked_b_slice(
                w13, bb, Int64(0), Int32(40), kt, start, tid, 128, self.width
            )
            stage_repacked_b_slice(
                w13,
                bb + self.width * 64,
            ...
            stage_repacked_sfb_slice(
                s13, sb, Int64(0), Int32(40), kt, start, tid, 128, self.width
            )
            stage_repacked_sfb_slice(
                s13,
                sb + self.width * 4,
            ...
            for kb in cutlass.range_constexpr(4):
                ...
                a3 = Uint32(0)
                if g < rows:
                    a0 = x[g, kt * 32 + kb * 8 + c * 2]
                    a2 = x[g, kt * 32 + kb * 8 + c * 2 + 1]
                if g + 8 < rows:
                    a1 = x[g + 8, kt * 32 + kb * 8 + c * 2]
                    a3 = x[g + 8, kt * 32 + kb * 8 + c * 2 + 1]
                scale = Uint32(0)
                if c < 2 and g + c * 8 < rows:
                    scale = Uint32(xs[g + c * 8, kt * 4 + kb])
        ...
        for ot in range(40):
            stage_repacked_b_k_slice(
                w2, bb, Int64(0), Int32(5), start, ot * 128, tid, 128, self.width
            )
            stage_repacked_sfb_k_slice(
                s2, sf, Int64(0), Int32(5), start, ot * 128, tid, 128, self.width
            )
            cute.arch.cp_async_commit_group()
            cute.arch.cp_async_wait_group(0)
            cute.arch.sync_threads()
