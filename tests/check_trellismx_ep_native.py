...
p.add_argument('--routing',choices=['owned','mixed','remote'],default='owned')
a=p.parse_args()
...
with torch.inference_mode():
 for m in (1,8,32,128,256):
  ...
  if a.routing!='remote':assert torch.count_nonzero(actual)
  if a.routing=='remote':
  ...
  print(json.dumps({'tokens':m,'routing':a.routing,'cosine':cosine,'relative_l2':relative_l2,'graph_replay_exact':True}),flush=True)
  assert cosine>=.999 and relative_l2<=.02
print(json.dumps({'status':'passed','scope':a.routing+'-expert numerical equivalence and graph replay only'}),flush=True)
