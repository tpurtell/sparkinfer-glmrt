...
from benchmarks import benchmark_compressed_sparse_mla


def _case(rows: int) -> benchmark_compressed_sparse_mla.BenchmarkCase:
    return benchmark_compressed_sparse_mla.BenchmarkCase(
        name="c128",
        rows=rows,
        swa_width=0,
        indexed_width=512,
...
def test_algorithm_cosine_threshold_is_hard_failure() -> None:
    ...
    benchmark_compressed_sparse_mla._check_algorithm_sanity(
        case,
        benchmark_compressed_sparse_mla.Sanity(max_abs=0.0, rmse=0.0, cos=0.995),
    )

    try:
        benchmark_compressed_sparse_mla._check_algorithm_sanity(
            case,
            benchmark_compressed_sparse_mla.Sanity(max_abs=0.0, rmse=0.0, cos=0.994999),
        )
    except benchmark_compressed_sparse_mla.BenchmarkFailure as exc:
...
def test_main_prints_no_geomean_on_cosine_failure(monkeypatch, capsys) -> None:
    def fake_collect_case_reports(args, *, device=None):
        del args, device
        raise benchmark_compressed_sparse_mla.BenchmarkFailure(
            "synthetic cosine failure"
        )

    monkeypatch.setattr(
        benchmark_compressed_sparse_mla,
        "collect_case_reports",
        fake_collect_case_reports,
    )

    rc = benchmark_compressed_sparse_mla.main([])
...
def test_target_summary_uses_rows1_and_rows4096_target_ratios() -> None:
    reports = [
        benchmark_compressed_sparse_mla.CaseReport(
            case=_case(1),
            replay_us=25.0,
            p90_replay_us=25.0,
    ...
    summary = benchmark_compressed_sparse_mla._compute_target_summary(reports)
    ...
    assert benchmark_compressed_sparse_mla._render_summary(reports, summary) == (
...
def test_parse_cases_accepts_distinct_c4_and_c128_widths() -> None:
    cases = benchmark_compressed_sparse_mla._parse_cases(
        "swa-c4,swa-c128",
        [1],
    ...
    assert benchmark_compressed_sparse_mla._planned_split_chunks(cases[1]) == 192


def test_parse_args_accepts_non_flash_local_q_heads() -> None:
    args = benchmark_compressed_sparse_mla._parse_args(["--num-q-heads", "16"])
...
def test_runtime_valid_widths_preserve_capture_capacity() -> None:
    ...
    assert [
        benchmark_compressed_sparse_mla._runtime_valid_widths(
...
def test_parse_args_accepts_runtime_context_length() -> None:
    args = benchmark_compressed_sparse_mla._parse_args(["--context-length", "16384"])

    assert args.context_length == 16_384


def test_shared_indexed_cache_uses_one_production_prefill_pool() -> None:
    case = benchmark_compressed_sparse_mla.BenchmarkCase(
        name="swa-c128",
        rows=4096,
        swa_width=128,
        indexed_width=8192,
    ...
    assert (
        benchmark_compressed_sparse_mla._indexed_cache_tokens(
            case,
    ...
    args = benchmark_compressed_sparse_mla._parse_args(["--shared-indexed-cache"])
    assert args.shared_indexed_cache
    assert (
        benchmark_compressed_sparse_mla._benchmark_workspace_mode(
            shared_indexed_cache=True
        )
        == "extend"
    )
...
def test_reused_cache_pool_preserves_decode_workspace() -> None:
    ...
    assert (
        benchmark_compressed_sparse_mla._swa_cache_tokens(
            case,
            shared_indexed_cache=False,
            reuse_cache_pool=True,
            cache_num_pages=7792,
    ...
    args = benchmark_compressed_sparse_mla._parse_args(["--reuse-cache-pool"])
...
def test_model_config_derives_live_dsv4_selected_widths() -> None:
    profile = (
        benchmark_compressed_sparse_mla._derive_dsv4_compressed_sparse_mla_profile(
            {
                "sliding_window": 128,
                "index_topk": 512,
...
def test_trace_preset_uses_full_packed_vllm_pool_geometry() -> None:
    args = benchmark_compressed_sparse_mla._parse_args(
        ["--preset", benchmark_compressed_sparse_mla.VLLM_DSV4_TRACE_PRESET]
    )
    ...
    span = (
        ...
        + benchmark_compressed_sparse_mla.compressed_sparse_mla_page_nbytes(
            args.swa_page_size
        )
    )
    assert span == benchmark_compressed_sparse_mla.VLLM_DSV4_TRACE_CACHE_VIEW_SPAN_BYTES


def test_cache_views_can_share_a_larger_strided_pool() -> None:
    page_size = 2
    page_nbytes = benchmark_compressed_sparse_mla.compressed_sparse_mla_page_nbytes(
        page_size
    )
    packed = torch.full((2, page_nbytes), 7, dtype=torch.uint8)
    backing = torch.zeros(4 * 4096, dtype=torch.uint8)

    views = benchmark_compressed_sparse_mla._make_cache_views(
        packed,
        page_size=page_size,
...
def test_trace_layer_weights_cover_all_target_layers() -> None:
    weights = benchmark_compressed_sparse_mla._trace_layer_weights(
        {
            "num_hidden_layers": 6,
...
def test_parse_model_cases_uses_runtime_variants() -> None:
    ...
    assert [
        benchmark_compressed_sparse_mla._planned_split_chunks(case) for case in cases
    ] == [
        11,
        54,
        235,
...
def test_target_summary_requires_both_target_rows() -> None:
    try:
        benchmark_compressed_sparse_mla._compute_target_summary(
            [
                benchmark_compressed_sparse_mla.CaseReport(
                    case=_case(1),
                    replay_us=25.0,
...
def test_main_prints_partial_case_reports_without_target_summary(
    ...
    rc = benchmark_compressed_sparse_mla.main(["--rows", "1"])
